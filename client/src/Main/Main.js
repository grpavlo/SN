...
const Main = ()=>{
    ...
    if(localStorage.getItem("SNnickName") ===null && localStorage.getItem("SNemail") ===null && localStorage.getItem("SNavatar") ===null && localStorage.getItem("SNpassword") ===null){
        ...
        if(!b){
            fetch(`http://185.237.204.110:3001/api/newPostDB`, {
                method: 'GET',
            }).then(response => {
        ...
        function exit(){
            localStorage.removeItem("SNnickName")
            localStorage.removeItem("SNpassword")
            window.open("/login","_self")
        }
        ...
        return(
            <div>
                ...
                <div className="topM">
                    ...
                    <button  className="buttonIconM" onClick={exit}> <img alt="img" className="iconM" src={icon5}/></button>
                </div>
